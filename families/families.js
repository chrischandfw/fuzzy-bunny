...
async function displayFamilies() {
    ...
    for (let family of families) {
        ...
        nameH3.textContent = family.name;
        // for each of this family's bunnies
        for (let bunny of family.fuzzy_bunnies) {
            const bunEl = document.createElement('div');
            
            bunEl.classList.add('bunny');
            bunEl.textContent = bunny.name;
			
            bunEl.addEventListener('click', async () => {
